            ...
            <div class="card-body">
                <form action="{{ route('mahasiswa.update', $mahasiswa->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="id" id="id" class="form-control @error('id') is-invalid @enderror" value="{{ old('id', $mahasiswa->id) }}">
                        @error('id')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="tel" name="no_telepon" id="no_telepon" class="form-control" value="{{ old('no_telepon', $mahasiswa->no_telepon) }}">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="prodi_id" id="prodi" class="form-select">
                            @foreach($prodis as $item)
                                <option value="{{ $item->id }}" {{ old("prodi_id" , $mahasiswa->prodi_id ) ==  $item->id ? 'selected' : '' }}>{{ $item->nama }}-{{ $item->jenjang }}</option>
                            @endforeach
                        </select>
                    </div>
