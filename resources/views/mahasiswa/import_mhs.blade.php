...
<style>
    ...
    table {
        width: 100%; /* Pastikan tabel memiliki lebar 100% */
    }
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <a class="btn btn-outline-primary float-end mb-1 "onClick="return confirm('Apakah sudah yakin?')" href="{{ route('mahasiswaimportsave') }}">Kirim<i
                class="bx bx-share"></i>
            </a>
             @endif
                ...
                <tbody class="table-border-bottom-0">
                    ...
                    @foreach ($mhs_temps as $bt)
                        <tr {!! $bt->check == "False" ? "style='background-color : goldenrod'" : '' !!}>
                            ...
                            <td {!! str_contains($bt->eror_location, 'F' . $row) ? "style='background-color: red'" : '' !!}>{{ $bt->prodi->nama }}
...
<script>
    $(document).ready(function () {
        ...
        table.columns(8).every(function () {
            var column = this;
            var select = $('<select class="form-select"><option value="" disabled selected>--Pilih Check--</option></select>')
